...
export function markWriting(): void {
  agentActivity.set({ state: "writing", lastEvent: Date.now() });
}

/** 外部变更处理完成(重载/rescan 结束) */
export function markDone(): void {
  agentActivity.set({ state: "done", lastEvent: Date.now() });
}
